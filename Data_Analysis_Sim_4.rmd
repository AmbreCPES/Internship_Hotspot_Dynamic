---
title: "Data Analysis: Measure of gBGC and LD"
...
* Ne = 10000
* mu = 2.25e-8 *(mean recombination rate)*
* r0 = 1e-7
* r = 25*r0
* hotspot length = 2000 bp *(in the center of the sequence)*
* Mutation matrix = matrix(c(0, 0.5e-8, 1.0e-8, 0.5e-8, 0.5e-8, 0, 0.5e-8, 2e-8, 2e-8, 0.5e-8, 0, 0.5e-8, 0.5e-8, 1.0e-8, 0.5e-8, 0), nrow=4, ncol=4, byrow=T )
...
for (param in 3:3){
  ...
  for (loci in 1423:2000){
    IN <- "/home/jaeger/sshfs-path_4/Simulation_4/2_Data_Slim/Out_Slim"
    
    
    folder <- "/home/jaeger/sshfs-path_4/Simulation_4"
...
for (param in 1:9) {
  ...
  N <- 1000
  file_HS <- paste("/home/jaeger/sshfs-path_4/Simulation_4/4_gBGC/Param_",param,"/HS.txt",sep="")
  file_FR <- paste("/home/jaeger/sshfs-path_4/Simulation_4/4_gBGC/Param_",param,"/FR.txt",sep="")
  folder <- paste("/home/jaeger/sshfs-path_4/Simulation_4/4_gBGC/Param_",param,sep="")
...
# bootstrap for the sets of parameters 1 to 5
  ...
  ggplot(mapping = aes(x = factor(Categorie, levels=c("-12.5","-25","-50","-100","-200")))) +
...
trace_recombination_map <- function(IN,param,L) {
  ...
  x1 <- c(1:L)
  y1 <- c(rep(0.002,times=as.integer(2*round(L/5))),rep(0.05,times=as.integer(round(L/5))),rep(0.002,times=as.integer(2*round(L/5))))
...
folder <- paste("/home/jaeger/sshfs-path_4/Simulation_4/3_Output_ldhelmet/Param_",param,sep="")
out <- paste("/home/jaeger/sshfs-path_4/Simulation_4/3_Output_ldhelmet/Concatenate_",param,sep="")

recombination_all(N,param,folder,out)


IN <- paste("/home/jaeger/sshfs-path_4/Simulation_4/3_Output_ldhelmet/Concatenate_",param,sep="")
OUT <- paste("/home/jaeger/sshfs-path_4/Simulation_4/3_Output_ldhelmet/Recombination_",param,sep="")
...
plot(0, type = "n", xlim = c(0, 10000), ylim = c(0, 0.01), xlab = "bp", ylab = expression(rho), bty = "n", col.axis = "black", col.lab = "black", bg = "white")
...
plot(0, type = "n", xlim = c(0, 10000), ylim = c(0, 0.03), xlab = "bp", ylab = expression(rho), bty = "n", col.axis = "black", col.lab = "black", bg = "white")
...
IN <- "/home/jaeger/sshfs-path_4/Simulation_4/3_Output_ldhelmet"
...
get_h <- function(folder,d,f,param) {
  ...
  for(i in d:f) {
    ...
    H <- data.frame(het_0=table_h[1,2],het_f_2=table_h[3,2])
    data <- rbind(data,H)
    }
  }
    write.table(data, file=paste("/home/jaeger/sshfs-path_4/Simulation_4/Het_",param,sep=""))
    return(data)
}

trace_mean_h <- function()
  #We trace the mean of het_0 and het_f from the Het files precendently created in the form of boxplots
{
 mean_het_0 <- c()
  ...
  for (i in 1:9) {
    file_name <- paste("/home/jaeger/sshfs-path_4/Simulation_4/Het_",i,sep="")
    table_h <- read.table(file_name,header=TRUE)
    mean_het_0 <- c(mean_het_0,mean(table_h$het_0))
    mean_het_f <- c(mean_het_f,mean(table_h$het_f_2))
  }
  print(mean_het_0)
  print(mean_het_f)
  ...
  labs(title = "Heterozygosity for Simulation 4",
       x = "Parameters Set",
...
```{r}
for (param in 1:9) {
  folder <- paste0("/home/jaeger/sshfs-path_4/Simulation_4/2_Data_Slim/Out_Slim/Param_",param)
get_h(folder,0,2000,param)
}
...
# Combine all dataframes into a single dataframe
  ...
  transform(paste0("df",i), Time = "Final", Value = table_h$het_f_2)
)
}
...
Seeing the lower $\Pi$ (2x lower approximately that in simulation 3), we can not conclude on the LD at the end of the simulation,because we close to the limit of LDhelmet resolution. Because of the difference in $\Pi$, we can compare the sets of parameters for this model, but we cannot compare with the model using a constant Ne. 